...
export class PatchyWorkerHost {
  ...
  async dispatch(message) {
    switch (message.method) {
      ...
      case "applyRetouchRepair": {
        const before = this.#snapshot();
        if (before.stateId !== BigInt(message.expectedStateId) ||
            before.revision !== BigInt(message.expectedRevision)) {
          const error = new Error("Retouch repair was prepared from a stale document state");
          error.name = "PatchyEngineError"; error.code = 6; throw error;
        }
        this.#engine.applyRetouchRepair(this.#requireSession(), before,
          { ...message, layerId: BigInt(message.layerId) },
          new Int32Array(message.cancellation));
        return this.#snapshot();
      }
      case "applyLocalAdjustmentBrush": {
        const before = this.#snapshot();
        if (before.stateId !== BigInt(message.expectedStateId) ||
            before.revision !== BigInt(message.expectedRevision)) {
          const error = new Error("Local-adjustment brush was prepared from a stale document state");
          error.name = "PatchyEngineError"; error.code = 6; throw error;
        }
        this.#engine.applyLocalAdjustmentBrush(this.#requireSession(), before,
          { ...message, layerId: BigInt(message.layerId) },
          new Int32Array(message.cancellation));
        return this.#snapshot();
      }
      case "applyAdvancedPaintStroke": {
        const before = this.#snapshot();
        if (before.stateId !== BigInt(message.expectedStateId) ||
            before.revision !== BigInt(message.expectedRevision)) {
          const error = new Error("Advanced-paint stroke was prepared from a stale document state");
          error.name = "PatchyEngineError"; error.code = 6; throw error;
        }
        this.#engine.applyAdvancedPaintStroke(this.#requireSession(), before,
          { ...message, layerId: BigInt(message.layerId) },
          new Int32Array(message.cancellation));
        return this.#snapshot();
      }
      ...
      case "previewSelectionRefinement": {
        const before = this.#snapshot();
        if (before.stateId !== BigInt(message.expectedStateId) ||
            before.revision !== BigInt(message.expectedRevision)) {
          const error = new Error("Selection refinement was prepared from a stale document state");
          error.name = "PatchyEngineError"; error.code = 6; throw error;
        }
